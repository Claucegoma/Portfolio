...
class PopupWindow extends LitElement {
  static sharedStyles = css`
            ...
            background: linear-gradient(160deg, rgba(47, 93, 197, 0.9) 0%, rgba(0, 0, 0, 0.9));
            border-radius: 20px;
            padding: 15px;
...
static styles = [PopupWindow.sharedStyles];
  ...
  connectedCallback() {
    ...
    document.body.addEventListener('click', this.cerrarVentanaSiEsExterior, true);
  }
  
  cerrarVentanaSiEsExterior = (event) => {
    console.log('Click fuera de la ventana');
    // Verifica si el clic no proviene de la ventana emergente ni de sus descendientes
    ...
    return html`
      <div class="content">
        ...
        <div class= "icons-logo">
          ${this.content.icons.map(icon => html`<img class="logo" src="${icon}" alt="">`)}
        </div>
      </div>
    `;
  }
...
`];
  ...
  constructor() {
    ...
    this.content = {
      image: "./assets/bqac.png",
      link1: "https://github.com/Claucegoma/DEV009-burger-queen-api-client/compare/main...AKMoralesRamos:DEV009-burger-queen-api-client:main",
